...
x_test,y_test=zip(*B)
dir_path='./'
result_file='result_grid.csv'
text_file = open(os.path.join(dir_path, result_file), 'w')
method_C1C2='weighted_shortest_path'
...
cv_scores = {}
for alpha in list(np.linspace(1,1500,4)):  
    for epsilon in list(np.linspace(0.01,50,3)): 
        start_time = time.time()
        clf = NN.Tree_OT_1NN_Classifier(alpha,epsilon
        	,the_lower_the_better=True,method=method_C1C2)
        l_scores = []
